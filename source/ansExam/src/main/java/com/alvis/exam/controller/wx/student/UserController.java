...
public class UserController extends BaseWXApiController {
    ...
    public RestResponse register(@Valid UserRegisterVM model) {
        User existUser = userService.getUserByMobile(model.getMobile());
        if (null != existUser) {
        ...
        user.setPhone(model.getMobile());
        userService.insertByFilter(user);
        UserEventLog userEventLog = new UserEventLog(user.getId(), user.getUserName(), user.getRealName(), new Date());
        userEventLog.setContent("欢迎 " + user.getUserName() + " 注册来到潘多拉考试系统");
        eventPublisher.publishEvent(new UserEvent(userEventLog));
        return RestResponse.ok();
    }
    @RequestMapping(value = "/updatePassword", method = RequestMethod.POST)
    public RestResponse updatePassword(@Valid UserRegisterVM model) {
        User user = modelMapper.map(model, User.class);
        String encodePwd = authenticationService.pwdEncode(model.getPassword());
        user.setPassword(encodePwd);
        user.setPhone(model.getMobile());
    ...
    public RestResponse<UserResponseVM> selectByPhone(@RequestParam String mobile) {
        if (StringUtils.isBlank(mobile)) {
            return RestResponse.fail(201,"手机号为空");
        }
    ...
    public RestResponse<List<ExamPaperAnswerPageResponseVM>> pageList() {

        List<ExamPaperAnswer> page = examPaperAnswerService.studentPage3(getCurrentUser().getId());
