...
public class ExamPaperServiceImpl extends BaseServiceImpl<ExamPaper> implements ExamPaperService {
    ...
    @Override
    public Integer editAllPaperFromVM(ExamPaperEditAllRequestVM examPaperEditAllRequestVM, User user) {
        ExamPaper examPaper = modelMapper.map(examPaperEditAllRequestVM, ExamPaper.class);
        List<String> dateTimes = examPaperEditAllRequestVM.getLimitDateTime();
        ...
        return examPaperMapper.updateAllExamPaper(examPaper);
    }
    ...
    @Override
    public List<Long> paperIDList() {
        return examPaperMapper.paperIDList();
    }

    @Override
    public boolean getUserCount(Integer userId) {
        if (examPaperMapper.getUserCount(userId) != 0 ){
            return false;
        }
        return true;
    }
    @Override
    public boolean getCount(Integer Id) {
        if (examPaperMapper.getCount(Id) != 0 ){
            return false;
        }
        return true;
    }
