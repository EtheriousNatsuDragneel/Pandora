...
export default {
  ...
  methods: {
    ...
    confirmQuestionSelect () {
      let _this = this
      let length =  _this.currentTitleItem.questionItems.length
      if (length > 0) {
        if (_this.booleanFlag(_this.questionPage.multipleSelection,_this.currentTitleItem.questionItems)) {
                _this.$msgbox({
                  title:'提示',
                  message:'题目重复'
                })
    ...
    levelChange () {
      ...
      this.subjectFilter = this.subjects.filter(data => data.level === 1)
    },
    search () {
