...
<!--        </el-select>-->
<!--      </el-form-item>-->
      <el-form-item label="类型：" prop="subjectId" required>
        <el-select v-model="form.subjectId" placeholder="类型" >
      ...
      <el-form-item label="填空答案：" required>
        <el-form-item :label="item.prefix" :key="item.prefix"  v-for="item in form.items"  label-width="50px" class="question-item-label">
          ...
          <el-button v-if="item.prefix != '1'" type="danger" size="mini" class="question-item-remove" icon="el-icon-delete" @click="deleteGap(item.prefix)"></el-button>
        </el-form-item>
      </el-form-item>
      ...
      <el-form-item>
        ...
        <el-button @click="addGap">增加填空</el-button>
      </el-form-item>
    </el-form>
...
export default {
  ...
  methods: {
    ...
    editorConfirm () {
      ...
      console.log(this.richEditor.parameterName)
    ...
    submitForm () {
      ...
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          // for (let i = 0; i < _this.form.items.length ; i++) {
          //   _this.form.correctArray.push(_this.form.items[i].content)
          // }
    ...
    addGap () {
      ...
      let id = parseInt(this.form.items[length - 1].prefix) + 1
      this.form.items.push({ id: null, prefix: '' + id, content: '', score: '' })
    },
    deleteGap (index) {
      this.form.items.splice(parseInt(index) - 1, 1)
        for (let i in this.form.items) {
          let j = parseInt(i) + 1
          this.form.items[i].prefix =j + ''
        }
    },
